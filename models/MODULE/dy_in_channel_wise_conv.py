...
class InvertedEx(nn.Module):
    def __init__(self, in_planes, ratios, in_C, K):
        ...
        self.fc2 = nn.Conv2d(hidden_planes, in_C*K, kernel_size=1, bias=True)
        self._initialize_weights()
...
class DyC_Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, 
        ...
        self.in_C = in_channels // groups
        self.se_module = InvertedEx(in_channels, ratio, self.in_C, K)
        self.weight = nn.Parameter(
            torch.randn(K, out_channels, self.in_C, kernel_size, kernel_size) * he_std
        )
    
    def forward(self, x):
        B, C, H, W = x.size()
        y = self.se_module(x)
        y = y.view(y.size(0), self.K, 1, self.in_C, 1)
        #prob = F.softmax(y/self.temperature, dim=1).unsqueeze(3)    
        prob = F.softmax(y/(self.hidden_planes ** 0.5), dim=1)
        print(prob.size())
        weight = self.weight.view(self.K, self.out_channels, self.in_C, -1).unsqueeze(0)
        aggregate_weight = (prob * weight).sum(dim=1)
        aggregate_weight = aggregate_weight.view(-1, self.in_C, self.kernel_size, self.kernel_size)
        x = x.view(1, -1, H, W)
