...
class SEModule(nn.Module):
    def __init__(self, in_planes, ratios, K, temperature):
        ...
        self.fc2_weight = nn.Parameter(torch.Tensor(K, hidden_planes))
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        #x = F.relu(x)
        x = F.linear(F.normalize(x.squeeze()), F.normalize(self.fc2_weight))
        x = x.view(x.size(0), -1)
...
class DyConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, 
        ...
        self.dropconnect = nn.Dropout2d(p=0.25)
        if bias:
    ...
    def forward(self, x):
        ...
        weight = self.dropconnect(self.weight)
        if self.groups == 1:
            #out = F.conv2d(x, weight=self.weight, bias=self.bias, stride=self.stride, 
            #            padding=self.padding, dilation=self.dilation, groups=self.groups)
            out = F.conv2d(x, weight=weight, bias=self.bias, stride=self.stride, 
                        padding=self.padding, dilation=self.dilation, groups=self.groups)
        else:
